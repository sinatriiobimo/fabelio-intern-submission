...
function Loading() {
  return Array(10).fill().map((_, index) => {
    <div className="px-4 relative card group" key={index}>
      <div className="rounded-xl bg-gray-300 overflow-hidden card-shadow relative" style={{width: 687, height: 386}}></div>
...
export default function Product() {
  ...
  const refContainer = useRef(null);
    ...
    return (
        <section class="flex flex-col py-16" id="click-to-product">
      <div class="container mx-auto mb-4">
        <div class="flex justify-start text-start mb-5">
          <h3 class="text-3xl capitalize text-gray-800 font-semibold">
            Rekomendasi Furniture
          </h3>
        </div>
      </div>
      <div class="overflow-x-hidden px-4" id="carousel">
        <div class="container mx-auto" ref={refContainer}></div>

          {isLoading ? (
            <div class="flex -mx-4 flex-row relative" style={{paddingLeft: refContainer.current?.getBoundingClientRect?.()?.left - 16 || 0}}><Loading/></div>
          ): error ? (
            JSON.stringify(error)
          ) : (
            <Carousel refContainer={refContainer}>
              {randomArrayShuffle(data).map(product => { 
                return (
                <div className="px-4 relative card group" key={product.id}>
                  <div className="rounded-xl overflow-hidden card-shadow relative" style={{width: 687, height: 386}}>
                    ...
                    <div className="space-x-1">
                      <h5 className="text-xl inline-block mt-4 capitalize">{product.productname}</h5>
                      <h5 className="text-md inline-block mt-4 text-bold text-gray-700 capitalize">({product.material})</h5>
                    </div>
                    <span className="text-lg font-semibold">{product.price.currency()}</span>
                    {product.sold == true ? (
                          <span className="square-box-red text-md text-center font-bold text-white mt-1 block capitalize">Sold</span>
                        ) : (
