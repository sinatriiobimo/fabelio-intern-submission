    ...
    return (
      ...
      <div class="container mx-auto mb-4">
        <div class="flex justify-start text-start mb-5">
          <h3 class="text-2xl capitalize text-gray-800 font-semibold">
            Product List
          </h3>
        </div>
      </div>
            ...
            <Carousel refContainer={refContainer}>
              {data.data.map((product) => { 
                ...
                <div className="px-4 relative card group" key={product.id }>
                  ...
                  <h5 className="text-lg font-semibold mt-4 capitalize">{product.productname}</h5>
                  <span className="text-md text-gray-600 capitalize">{product.colours}</span>
                  <br></br>
                  <span className="text-xl">Rp {product.price}</span>
                  <Link to={`/categories/${product.id}/products/${product.id}`} className="stretched-link"></Link>
              </div>
              )})}
            </Carousel>
          )}
