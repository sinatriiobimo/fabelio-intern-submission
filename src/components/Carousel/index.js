...
import { addClass, removeClass } from 'helpers/modifier';

export default function index({children, refContainer}) {
    const refDragHandler = useRef(null);
    const containerClientRect = refContainer.current.getBoundingClientRect()
    const [index, setIndex] = useState(0);

    const threshold = 100
    const itemToShow = window.innerWidth < 767 ? 1 : 1
    ...
    return (
        <div class="flex -mx-4 flex-row relative" ref={refDragHandler} style={{paddingLeft: containerClientRect.left - 16}}>
            {children}
        </div>
    )
}
